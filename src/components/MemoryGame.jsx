...
return (
    <div style={{
      backgroundColor: '#0f172a', borderRadius: '16px', color: '#fff',
      ...
      {/* EN-TÊTE FIXE */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '10px 15px', flex: 'none', backgroundColor: 'rgba(0,0,0,0.2)' }}>
        <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '32px', cursor: 'pointer', color: '#888', display: 'flex' }}>✖</button>
      </div>

      {/* CORPS SCROLLABLE (contenant le jeu ou l'écran de fin/chargement) */}
      <div className="hide-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '0 15px 20px 15px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', padding: '40px 0' }}>
            ...
            <div style={{ fontSize: 'clamp(1.2rem, 4vw, 1.5rem)', color: '#f59e0b' }}>MÉLANGE DU DECK...</div>
            <div style={{ fontSize: '1rem', color: '#64748b' }}>Préparez vos réflexes</div>
          </div>
        ) : isGameOver ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', padding: '20px 0' }}>
            <div style={{ fontSize: '4rem' }}>🎉</div>
            <h2 style={{ color: '#f59e0b', margin: 0, fontSize: 'clamp(1.8rem, 6vw, 2.5rem)' }}>VICTOIRE !</h2>
            <p style={{ fontSize: '1.2rem' }}>Deck complété en <span style={{ color: '#f59e0b' }}>{moves}</span> coups.</p>
            <button onClick={onClose} style={{ backgroundColor: '#f59e0b', padding: '12px 30px', borderRadius: '30px', border: 'none', cursor: 'pointer', color: '#0f172a', fontSize: '1.2rem', transition: 'transform 0.2s' }}>QUITTER</button>
          </div>
        ) : (
          <>
            <h2 style={{ fontSize: 'clamp(1.5rem, 5vw, 2rem)', color: '#f59e0b', margin: 0, textTransform: 'uppercase' }}>Memory</h2>
            <div style={{ fontSize: '1.1rem', backgroundColor: '#1e293b', padding: '5px 15px', borderRadius: '20px', border: '1px solid #334155' }}>
              Coups : <span style={{ color: '#f59e0b' }}>{moves}</span> | Paires : <span style={{ color: '#f59e0b' }}>{matched.length / 2}</span> / 9
            </div>
              ...
              {cards.map((card, index) => {
                ...
                return (
                  <div key={index} onClick={() => handleCardClick(index)} style={{
                    ...
                    transition: 'transform 0.4s, background-color 0.2s', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)',
                  }}>
                    {isFlipped ? (
